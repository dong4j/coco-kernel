...
import io.github.dong4j.coco.kernel.common.enums.SerializeEnum;
...
public class EntityEnumDeserializer<T extends SerializeEnum<?>> extends JsonDeserializer<T> implements ContextualDeserializer {
    ...
    private Class<T> clz;
    /** 缓存枚举反序列化器 */
    private static final Map<String, EntityEnumDeserializer<? extends SerializeEnum<?>>> DESERIALIZER_MAP = new ConcurrentHashMap<>();
    /** 缓存枚举值 */
    private static final Map<String, Map<Serializable, SerializeEnum<?>>> ALL_ENUM_MAP = new HashMap<>();
    /** EMPTY_JSON */
    private static final String EMPTY_JSON = StrPool.EMPTY_JSON;
    ...
    private static @Nullable Serializable getValue(JsonNode node) {
        ...
        switch (jsonNodeType) {
            ...
            case MISSING:
            default:
                throw new KernelException(StrFormatter.format("不支持的枚举转换: {}", node.toString()));
        }
        return value;
    }
    ...
    public JsonDeserializer<?> createContextual(@NotNull DeserializationContext ctx, BeanProperty property) {
        ...
        if (entityEnumJsonDeserializer == null) {
            ...
            Map<Serializable, SerializeEnum<?>> currentEnumMap = new ConcurrentHashMap<>(enums.length);
            for (T e : enums) {
