...
import io.github.dong4j.coco.kernel.common.util.JustOnceLogger;
...
public final class App {
    ...
    /** 启动标识, 所有项目只能使用 CocoApplication 启动 */
    public static final String START_COCO_APPLICATION = "start_coco_application";
    /** 依赖的 library */
    ...
    /** 由 coco-assist-maven-plugin 自动生成 */
    ...
    public static class Components {
        /** COCO_REST_SPRING_BOOT */
        public static final String COCO_REST_SPRING_BOOT = "coco-rest-spring-boot";
        /** COCO_AGENT_SPRING_BOOT */
        public static final String COCO_AGENT_SPRING_BOOT = "coco-agent-spring-boot";
    }

    /**
     * 本地开发时, 优先从 JVM 环境变量中读取 {@link SystemUtils#USER_NAMESPACE_VALUE}, 然后是 user.name,
     * 如果 {@link SystemUtils#USER_NAME} 是 {@link App#WINDOWS_DEFAULT_USER_NAME}, 则读取系统环境变量 COCO_NAME_SPACE).
     * 如果通过 shell 启动, 会自动设置 COCO_NAME_SPACE 为传入的 env 参数, 不会会加上 coco- 前缀, 最终找到 coco-{env} 的 nacos namespace.
     */
    public static final String COCO_NAME_SPACE;
    ...
    static {
        ...
        // 第二优先级是 COCO_NAME_SPACE
        if (StrUtil.isBlank(currentEnv)) {
            currentEnv = StrUtil.isBlank(SystemUtils.getProperty("COCO_NAME_SPACE"))
                         ? StrUtil.EMPTY
                         : SystemUtils.getProperty("COCO_NAME_SPACE");
        }

        // 第三优先级是 user.name, 如果在 windows 未设置计算机名, 此名称将是 Administrator
        currentEnv = StrUtil.isBlank(currentEnv)
                     ? SystemUtils.USER_NAME
                     : currentEnv;

        // 如果是 Administrator 则获取 COCO_NAME_SPACE
        if (WINDOWS_DEFAULT_USER_NAME.equalsIgnoreCase(currentEnv)) {
            JustOnceLogger.printOnce(App.class.getName(), "请配置环境变量 「COCO_NAME_SPACE」 或修改本地的计算机名. "
                                                          + "如果需要在本地开发时修改 namespace, 请设置 JVM 参数 「user.namespace」"
                                                          + "或直接使用 System.setProperty(SystemUtils.USER_NAMESPACE, \"xxxx\"))");
            currentEnv = "public";
        }

        COCO_NAME_SPACE = currentEnv;
    }
