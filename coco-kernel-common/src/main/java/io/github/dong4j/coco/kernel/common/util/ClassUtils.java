...
public class ClassUtils extends ClassUtil {
    ...
    public static @NotNull Class<?> getInterfaceT(@NotNull Class<?> clazz, Class<?> interfaceClass, int index) {
        ...
        if (targetType == null) {
            throw new KernelException("[{}] 未实现 [{}] 接口", clazz, interfaceClass);
        }
    ...
    private static @NotNull Class<?> checkType(Type type, int index) {
        if (type instanceof Class<?>) {
            ...
            throw new KernelException("Expected a Class, ParameterizedType" + ", but <" + type + "> is of type " + className);
        }
    }
    ...
    public static <T> @NotNull T newInstance(@NotNull Class<T> clazz) {
        try {
            ...
            throw new KernelException("实例化对象时出现错误,请尝试给 %s 添加无参的构造方法", e, clazz.getName());
        }
    }
    ...
    public static @NotNull Class<?> toClassConfident(String name) {
        try {
            ...
            throw new KernelException("找不到指定的class！请仅在明确确定会有 class 的时候, 调用该方法", e);
        }
    }
