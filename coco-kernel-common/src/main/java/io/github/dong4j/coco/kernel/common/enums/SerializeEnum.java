...
import io.github.dong4j.coco.kernel.common.annotation.SerializeValue;
import io.github.dong4j.coco.kernel.common.enums.serialize.EntityEnumDeserializer;
import io.github.dong4j.coco.kernel.common.enums.serialize.EntityEnumSerializer;
import io.github.dong4j.coco.kernel.common.exception.KernelException;
import io.github.dong4j.coco.kernel.common.util.EnumUtils;
...
public interface SerializeEnum<T extends Serializable> {
    ...
    static <E extends Enum<E> & SerializeEnum<?>> E valueOf(Class<E> clazz, Serializable value) {
        String errorMessage = StrFormatter.format("枚举类型转换错误: 没有找到对应枚举. value = {}, SerializeEnum = {}",
                                                  value,
                                                  clazz);
        return EnumUtils.of(clazz, e -> e.getValue().equals(value)).orElseThrow(() -> new KernelException(errorMessage));
    }
    ...
    static <T> @NotNull T getEnumByNameOrOrder(Class<? extends Enum<?>> clz, Serializable finalValue) {
        ...
        if (getForName.isPresent()) {
            ...
            throw new KernelException(StrFormatter.format("未找到匹配的枚举: [{}]", value));
        }
        return result;
    }
    ...
    static <T> T getEnumByOrder(Class<? extends Enum<?>> clz, String value) {
        ...
        try {
            ...
            throw new KernelException("通过枚举下标查找枚举出错: [{}] 无法转换为下标", value);
        }
        // 使用下标匹配
        Optional<? extends Enum<?>> getForOrdinal = EnumUtils.of(clz, e -> e.ordinal() == index);
        return (T) getForOrdinal.orElseThrow(() -> new KernelException("无法将 [{}] 转换为 [{}]", value, clz));
    }
