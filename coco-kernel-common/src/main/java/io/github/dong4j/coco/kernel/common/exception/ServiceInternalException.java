...
public class ServiceInternalException extends RuntimeException {
    ...
    public String getMessage() {
        return super.getMessage()
               + (StrUtil.isBlank(this.applicationName) ? "" : ": applicationName='" + this.applicationName + '\'')
               + (StrUtil.isBlank(this.ip) ? "" : ", ip='" + this.ip + '\'')
               + (this.port == null ? "" : ", port='" + this.port + '\'')
               + (StrUtil.isBlank(this.env) ? "" : ", env='" + this.env + '\'')
               + (StrUtil.isBlank(this.traceId) ? "" : ", traceId='" + this.traceId);
    }
