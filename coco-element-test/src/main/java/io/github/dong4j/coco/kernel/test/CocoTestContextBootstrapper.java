 ...
 */
@Slf4j
class CocoTestContextBootstrapper extends SpringBootTestContextBootstrapper {
    ...
    protected Class<? extends ContextLoader> getDefaultContextLoaderClass(Class<?> testClass) {
        return CocoSpringBootContextLoader.class;
    }
    ...
    protected String[] getProperties(Class<?> testClass) {
        ...
        return MergedAnnotations.from(testClass, SearchStrategy.INHERITED_ANNOTATIONS).get(CocoTest.class)
            .getValue("properties", String[].class).orElse(new String[0]);
    ...
    protected SpringBootTest.WebEnvironment getWebEnvironment(Class<?> testClass) {
        CocoTest annotation = this.getCocoTestAnnotation(testClass);
    ...
    /**
     * Gets coco test annotation *
     *
     * @param testClass test class
     * @return the coco test annotation
     * @since 1.0.0
     */
    private CocoTest getCocoTestAnnotation(Class<?> testClass) {
        return MergedAnnotations.from(testClass, SearchStrategy.INHERITED_ANNOTATIONS).get(CocoTest.class)
    ...
    protected void verifyConfiguration(Class<?> testClass) {
        ...
        CocoTest springBootTest = this.getCocoTestAnnotation(testClass);
        ...
        if (springBootTest != null
            ...
            throw new IllegalStateException("@WebAppConfiguration should only be used "
                                            + "with @CocoTest when @CocoTest is configured with a "
                                            + "mock web environment. Please remove @WebAppConfiguration or reconfigure @CocoTest.");
        }
    }
